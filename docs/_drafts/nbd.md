...
for [Network Block Device (NBD)][0], caught my eye. I played around with NBD,
this time I only looked at existing libraries and didn't implement hte protocol
myself.

## Experiment

The QEMU project includes a Network Block Device server called [`qemu-nbd`][1].
The server can export a QEMU disk image using the [NBD protocol][2]. The
[documentation][1] for it has several examples including how to set-up the
use of a certificate for authentication but I'm just interested in the
basics, so will overlook that.
...
* After the first successful client disconnects from the server, the server
  will exit.
...
The obvious client for the above server is QEMU itself. QEMU supports Network
Block Devices using TCP protocol and Unix Domain Sockets.

A device can be provided to a qemu-system executable using TCP as follows:
```--drive file=nbd://192.0.2.1:30000```
If the export was named then the name of the export forms the path, i.e.
`/qemu` based on the example above.

My interest in NBD was for using it over the network so the Unix Domain Sockets
weren't of interested at this time.

The Linux Kernel has a [module][3] for using NBD over the network, this is
what will be used to test out the server above.
...
format the device to whatever format. In my case I'm going to `vfat` as
...
author's own [library/crate][5] for working with NBD. This was ideal as the
licence is MIT/Apache 2.0.
...
* The production quality implementation with the kitchen sink (it provides
  ...
  which is AGPL3 with option for commercial licensing.
...
### Patch
As part of my playing around, I submitted a [patch][8] for `nbdserv` to update
the `nbd` crate as the API changed in order to support multiple exports. This
marks the first pull request that I submitted for a project in Rust.

## Alternative Client
I was unable to get QEMU with direct Linux boot plus storage from a drive from
NBD working.
```
...
machine (VM) disk images. The former lets you examine and modifier virtual
...
After mounting the file system which was formatted as ext3, it started reading
the smaller parts, i.e. 1024 bytes from here and there.
...
Several tools that I used to help chased down problems with the more complicated
NBD storage option, I won't go into how that project worked here but these were
useful for testing and diagnosing issues.
...
[NBDKit][10] plugin where by data is stored by red stone in a Minecraft world.
